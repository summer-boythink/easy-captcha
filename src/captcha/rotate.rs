...
impl RotateAppState {
    ...
    pub fn generate_captcha(&self) -> String {
        ...
        // 计算圆形区域的直径和中心点
        let diameter = std::cmp::min(img.width(), img.height()) - 1;
        let radius = diameter / 2;
        let center_x = img.width() / 2;
        let center_y = img.height() / 2;

        // 创建一个全透明的图像
        let mut circle_img = ImageBuffer::from_pixel(diameter, diameter, Rgba([255, 255, 255, 0]));
        ...
        let rotated = rotate_about_center(
            &DynamicImage::ImageRgba8(circle_img).to_rgb8(),
        ...
        // 移除旋转后的图像周围的透明边缘
        let mut final_img = ImageBuffer::from_pixel(diameter, diameter, Rgb([255, 255, 255]));
        for y in 0..diameter {
            for x in 0..diameter {
                if (
                    rotated.get_pixel(x, y)[0],
                    rotated.get_pixel(x, y)[1],
                    rotated.get_pixel(x, y)[2],
                ) != (0, 0, 0)
                {
                    let pixel = rotated.get_pixel(x, y).to_rgb();
                    final_img.put_pixel(x, y, pixel);
                }
            }
        }
        // 调整大小
        let resized_img = DynamicImage::ImageRgb8(final_img).resize_exact(
        ...
        let hash_key: String = (0..16).map(|_| rng.sample(Alphanumeric) as char).collect();
        self.check_map.lock().unwrap().insert(
            hash_key.clone(),
            (random_angle - leeway, random_angle + leeway),
        );
        "data:image/png;base64,".to_owned() + &BASE64_STANDARD.encode(&buffer.into_inner())
    }
